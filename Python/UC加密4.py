#!/usr/bin/env python3
"""
Unicode 范围加密工具 (版本 4.0) - 包含命令行和图形界面
...
class Encryptor:
    ...
    def _keystream(self, enc_key: bytes, nonce: bytes, counter: int, length: int) -> bytes:
        """生成第 counter 块所需的密钥流（长度 ≤ BLOCK_SIZE）"""
        counter_bytes = counter.to_bytes(4, 'big')
        prf_input = nonce + counter_bytes
        full = hmac.new(enc_key, prf_input, hashlib.sha256).digest()
        return full[:length]
...
def encrypt_file(input_path: str, output_path: str, password: str, compress: bool):
    ...
    salt = os.urandom(SALT_LEN)
    nonce = os.urandom(NONCE_LEN)
    enc_key, mac_key = enc._derive_keys(salt)

    # 处理压缩
    if compress:
        # 简单方案：整体压缩（压缩后通常较小，内存压力可接受）
        with open(input_path, 'rb') as f:
            data = f.read()
        data_to_encrypt = zlib.compress(data)
        flag = b'\x01'
        # 直接调用原有的加密方法（一次处理）
        ciphertext = enc._encrypt_stream(data_to_encrypt, enc_key, nonce)
        signature = hmac.new(mac_key, ciphertext, hashlib.sha256).digest()
        packed = MAGIC + flag + salt + nonce + signature + ciphertext
        ...
        with open(input_path, 'rb') as fin, open(temp_path, 'wb') as fout:
            ...
            while True:
                ...
                # 生成当前块的密钥流
                keystream = enc._keystream(enc_key, nonce, counter, len(chunk))
                cipher_chunk = bytes(a ^ b for a, b in zip(chunk, keystream))
...
def decrypt_file(input_path: str, output_path: str, password: str):
    ...
    # 读取整个编码文件（解码后可能较大，但编码文件是文本，通常可接受）
    # 如果担心编码文件过大，也可以流式解码，但实现复杂，暂不处理
    ...
    salt = packed[offset+1:offset+1+SALT_LEN]
    nonce = packed[offset+1+SALT_LEN:offset+1+SALT_LEN+NONCE_LEN]
    sig_start = offset+1+SALT_LEN+NONCE_LEN
    sig_end = sig_start + SIG_LEN
    signature = packed[sig_start:sig_end]
    ciphertext = packed[sig_end:]
    ...
    # 处理压缩标志
    if flag == b'\x01':
        # 压缩：一次性解密并解压（密文已全部在内存中）
        plain_compressed = enc._decrypt_stream(ciphertext, enc_key, nonce)
        ...
        with open(output_path, 'wb') as fout:
            ...
            while pos < total_len:
                chunk_size = min(CHUNK_SIZE, total_len - pos)
                chunk = ciphertext[pos:pos+chunk_size]
                keystream = enc._keystream(enc_key, nonce, counter, len(chunk))
                plain_chunk = bytes(a ^ b for a, b in zip(chunk, keystream))
